...
class RealsenseConfig:
    ...
    img_height = 360  # Image height in pixels
...
class CameraTrans:
    ...
    def camera_to_image(self, P_camera: np.ndarray) -> np.ndarray:
        ...
        Supports distortion if 'D' is present in intrinsics config.
        ...
        # Apply distortion if coefficients exist
        if hasattr(self.intrinsics_cfg, "D") and self.intrinsics_cfg.D:
            D = self.intrinsics_cfg.D
            r2 = x * x + y * y

            # Assuming D is [k1, k2, k3, k4] (fisheye) or [k1, k2, p1, p2] (standard)
            # Given 4 params, let's assume standard OpenCV [k1, k2, p1, p2] model first as it's most common for pinhole
            # If it's fisheye, the model is different (theta = atan(r)).
            # Realsense D435i RGB is usually standard model.
...
def verify_camera_transform(camera: CameraTrans, num_points: int = 10) -> float:
    """
    ...
    Args:
        camera (CameraTrans): Camera object.
...
if __name__ == "__main__":
    ...
    cam = CameraTrans(cfg=RealsenseConfig())
